...
document.addEventListener("DOMContentLoaded", () => {
    fetch("https://raw.githubusercontent.com/samapau-bu/asynchronous-web-development/main/item.json?token=GHSAT0AAAAAACSZI5RMICEJUXXNQIYT7ELGZTFHWOA")
    ...
    function createDraggableItem(item, category) {
        ...
        document.getElementById(category).appendChild(draggableElement);
    }
    ...
    dropzones.forEach(dropzone => {
        ...
        dropzone.addEventListener("drop", (event) => {
            ...
            if (category === dropzone.id) {
                dropzone.appendChild(document.querySelector(`[data-category="${category}"][draggable="true"]`));
            }
        });
    });
});
